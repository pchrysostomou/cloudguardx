...
export class CloudGuardApiClient {
  ...
  connectAwsAccount(dto: {
    name: string;
    externalAccountId: string;
    roleArn: string;
    externalId: string;
  }): Promise<CloudAccountSummary> {
    return this.request<CloudAccountSummary>("/cloud-accounts/aws", {
      method: "POST",
...
export function refreshTokenFromStorage(storage: Storage = window.localStorage): string | null {
  return storage.getItem("cloudguardx.refreshToken");
}
...
export function saveTokensToStorage(
  ...
  storage.setItem("cloudguardx.accessToken", accessToken);
  storage.setItem("cloudguardx.refreshToken", refreshToken);
}

export function clearTokensFromStorage(storage: Storage = window.localStorage): void {
  storage.removeItem("cloudguardx.accessToken");
  storage.removeItem("cloudguardx.refreshToken");
}
